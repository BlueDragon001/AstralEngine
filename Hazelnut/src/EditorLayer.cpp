...
namespace Hazel {
	...
	{
		std::string filepath = FileDialogs::OpenFile("Astral Scene (*.astral)\0*.astral\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".astral")
		{
	...
	{
		std::string filepath = FileDialogs::SaveFile("Astral Scene (*.astral)\0*.astral\0");
		if (!filepath.empty())
		{
